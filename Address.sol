...
operation.
    ...
    /**
     * @dev A call to an address target failed. The target may have 
reverted.
     */
    ...
    function sendValue(address payable recipient, uint256 amount) internal 
{
     ...
     * function instead.
     *
     * If `target` reverts with a revert reason or custom error, it is 
bubbled
     * up by this function (like regular Solidity function calls). 
However, if
     * the call reverted with no returned reason, this function reverts 
with a
...
successful, and reverts if the target
     * was not a contract or bubbling up the revert reason (falling back 
to {FailedInnerCall}) in case of an
     * unsuccessful call.
     */
    function verifyCallResultFromTarget(
        ...
        if (!success) {
            ...
            // only check if target is a contract if the call was 
...
reverts if it wasn't, either by bubbling the
