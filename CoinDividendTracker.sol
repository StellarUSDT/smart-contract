...
contract TruncatedERC20 {
  ...
  /**
   * @dev Indicates an error related to the current `balance` of a 
...
interface DividendPayingTokenOptionalInterface {
  ...
  function withdrawableDividendOf(address _owner) external view returns 
(uint256);

  function withdrawnDividendOf(address _owner) external view returns 
(uint256);

  function accumulativeDividendOf(address _owner) external view returns 
(uint256);
}

/// @title Dividend-Paying Token
/// @author Roger Wu (https://github.com/roger-wu)
/// @dev A mintable ERC20 token that allows anyone to pay and distribute 
ether
/// to token holders as dividends and allows token holders to withdraw 
their dividends.
/// Reference: the source code of PoWH3D: 
https://etherscan.io/address/0xB3775fB83F7D12A36E0475aBdD1FCA35c091efBe#code
contract DividendPayingToken is TruncatedERC20, 
...
TruncatedERC20(_name, _symbol) {}
  ...
  function distributeDividends() public payable {
    ...
    if (msg.value > 0) {
      magnifiedDividendPerShare = magnifiedDividendPerShare + (msg.value * 
magnitude / totalSupply());
  ...
  function _withdrawDividend(address account, bool autoClaim) internal 
returns(uint256) {
...
withdrawableDividend;
      ...
      bool success = autoClaim ? _sendDividendAuto(account, 
withdrawableDividend) : _sendDividendManual(account, 
withdrawableDividend);
...
withdrawableDividend;
  ...
  function _sendDividendAuto(address account, uint256 value) internal 
returns(bool success) {
    (success,) = payable(account).call{value: value, gas: 
callbackGas}("");
  }

  function _sendDividendManual(address account, uint256 value) internal 
returns(bool success) {
    (success,) = payable(account).call{value: value}("");
  }

  function dividendOf(address account) public view override 
returns(uint256) {
    return withdrawableDividendOf(account);
  }

  function withdrawableDividendOf(address account) public view override 
returns(uint256) {
    return accumulativeDividendOf(account) - withdrawnDividends[account];
  }

  function withdrawnDividendOf(address account) public view override 
returns(uint256) {
    return withdrawnDividends[account];
  }

  function accumulativeDividendOf(address account) public view override 
returns(uint256) {
    return ((magnifiedDividendPerShare * 
balanceOf(account)).toInt256Safe() + 
magnifiedDividendCorrections[account]).toUint256Safe() / magnitude;
  }
...
value).toInt256Safe();
  ...
  function _setBalance(address account, uint256 newBalance) internal {
    ...
    if (newBalance > currentBalance) _mint(account, newBalance - 
currentBalance);
    else if (newBalance < currentBalance) _burn(account, currentBalance - 
newBalance);
  }
}
...
library IterableMapping {
  ...
  function getIndexOfKey(Map storage map, address key) public view returns 
(int) {
    if(!map.inserted[key]) {
        return -1;
    }
...
contract DividendTracker is Ownable, DividendPayingToken {
  ...
  constructor(uint256 _claimWait, uint256 _minimumTokenBalance, uint256 
_callbackGas) DividendPayingToken("DividendTracker", "DividendTracker") 
Ownable(msg.sender) {
  ...
  function callbackGasSetup(uint256 newCallbackGas) public onlyOwner {
    if (newCallbackGas < 2_300 || newCallbackGas > 10_000) revert 
InvalidCallbackGas(newCallbackGas);
  ...
  function excludeFromDividends(address account, uint256 balance, bool 
isExcluded) external onlyOwner {
    if (isExcluded) {
      if (isExcludedFromDividends[account]) revert 
AlreadyExcludedFromDividends(account);
      ...
      if (!isExcludedFromDividends[account]) revert 
AlreadyIncludedInDividends(account);
  ...
  function claimWaitSetup(uint256 newClaimWait) public onlyOwner {
    if (newClaimWait < 60 || newClaimWait > 7 days) revert 
InvalidClaimWait(newClaimWait);
...
nextClaimTime - block.timestamp : 0;
  ...
  {
    if (index >= tokenHoldersMap.size()) return (address(0), -1, -1, 0, 0, 
0, 0, 0);
  ...
  function claim(address account, bool autoClaim) public onlyOwner returns 
(bool) {
  ...
  function _canAutoClaim(uint256 lastClaimTime) private view returns 
(bool) {
...
onlyOwner {
  ...
  function process(uint256 gas) external onlyOwner returns(uint256 
iterations, uint256 claims) {
...
_lastProcessedIndex = 0;
      ...
      if (gasLeft > newGasLeft) gasUsed = gasUsed + (gasLeft - 
newGasLeft);
...
abstract contract DividendTrackerFunctions is Ownable2Step {
  ...
  function _deployDividendTracker(uint256 claimWait, uint256 
minimumTokenBalance, uint256 callbackGas) internal {
...
callbackGas);
  ...
  function gasForProcessingSetup(uint256 newGasForProcessing) public 
onlyOwner {
    if (newGasForProcessing < 200_000 || newGasForProcessing > 500_000) 
revert InvalidGasForProcessing(newGasForProcessing);
...
internal virtual;
  ...
  function isExcludedFromDividends(address account) public view returns 
(bool) {
  ...
  function getTotalDividendsDistributed() external view returns (uint256) 
{
  ...
  function withdrawableDividendOf(address account) public view returns 
(uint256) {
