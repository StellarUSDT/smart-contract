...
// Remastered from OpenZeppelin Contracts (last updated v5.0.0) 
...
library SafeERC20Remastered {
    ...
    /**
     * @dev Transfer `value` amount of `token` from the calling contract 
to `to`. If `token` returns no value,
...
to `to`. If `token` returns no value,
     * non-reverting calls are assumed to be successful.
     */
    function safeTransfer_noRevert(IERC20 token, address to, uint256 
...
abi.encodeCall(token.transfer, (to, value)));
    ...
    /**
     * @dev Transfer `value` amount of `token` from `from` to `to`, 
spending the approval given by `from` to the
     * calling contract. If `token` returns no value, non-reverting calls 
are assumed to be successful.
     */
    function safeTransferFrom(IERC20 token, address from, address to, 
uint256 value) internal {
        _callOptionalReturn(token, abi.encodeCall(token.transferFrom, 
(from, to, value)));
    }
...
`value`. If `token` returns no value,
     * non-reverting calls are assumed to be successful. Meant to be used 
with tokens that require the approval
...
to perform this call, which verifies that
